<div>
    ...
    @else
    @php
    $statusProperties = $this->getStatusProperties();
    $statusLabel = $statusProperties[$order->status]['label'] ?? ucfirst($order->status);
    $statusColor = $statusProperties[$order->status]['color'] ?? '';
    @endphp

    <p class="inline-flex items-center px-4 py-2 font-bold transition border rounded" style="background-color: {{ $statusColor }}; color: white;">
    ...
    <x-hub::slideover title="Update status" form="updateStatus" wire:model="showStatusSelect">
        <div class="space-y-4">
            ...
            @else
            <x-hub::input.group :label="__('adminhub::components.orders.status.additional-content.label')" :instructions="__('adminhub::components.orders.status.additional-content.instructions')" for="additionalContent" :error="$errors->first('additionalContent')">
                <x-hub::input.textarea wire:model="additionalContent" rows="4" />
            </x-hub::input.group>

            <x-hub::input.group :label="__('adminhub::components.orders.status.mailers.label')" :instructions="__('adminhub::components.orders.status.mailers.instructions')" for="selectedMailers" :error="$errors->first('selectedMailers')">
    ...
    <x-hub::slideover :title="__('adminhub::components.orders.status.preview.title')" wire:model="previewTemplate" class="!max-w-4xl">
        <div class="space-y-4 h-full">
            ...
            <div class="h-full flex-col" x-data="{
                ...
                <iframe srcdoc="{{ $this->previewHtml }}" class="grow w-full h-full" x-ref="iframe"></iframe>
            </div>
        </div>
    </x-hub::slideover>
</div>